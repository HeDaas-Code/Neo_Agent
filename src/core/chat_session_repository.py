"""
ChatSessionRepository - 聊天会话数据访问层
=====================================

v3.1.0 新增；提供 chat_sessions / chat_messages 两张表的 CRUD。

设计要点：
1. 不直接持有 SQLite 连接，全部走 ``db_manager.get_connection()`` 上下文管理器
   —— 这样 WAL / 外键约束 / 异常回滚等基础设施被复用，避免行为分叉。
2. 写入操作（add_message / update_message_emotion）会自动调 ``touch()`` 维护
   ``updated_at`` 和 ``message_count``，调用方无需关心缓存字段。
3. 删除 session 依赖 ``chat_messages.session_id`` 的 ``ON DELETE CASCADE``，
   单条 DELETE 即可级联清空所有 messages。
4. 字段映射：INTEGER PRIMARY KEY -> int；REAL 时间戳用 ``time.time()``；
   TEXT 直接存 str（emotion_json 已经是 JSON 字符串）。
"""
