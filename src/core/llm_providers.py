"""
LLM Providers - v3.1.0 供应商注册表
=====================================

提供 6 个 OpenAI-兼容 preset + 统一的环境变量解析入口。

优先级（向后兼容）：
- LLM_API_KEY / LLM_BASE_URL / LLM_MODEL_NAME 是新主键
- 旧 SILICONFLOW_API_KEY / SILICONFLOW_API_URL 仍能 fallback（旧配置不破）
- LLM_PROVIDER 未设时：探测 SILICONFLOW_API_URL 是否含 siliconflow.cn，
  或 SILICONFLOW_API_KEY 已设 → 返 siliconflow；否则返 openai

注意：
- ``custom`` 预设的 base_url / default_model 为 None —— 调用方必须显式提供
  LLM_BASE_URL / LLM_MODEL_NAME，否则 raise ConfigError。
- 所有函数都是纯函数 + 读环境变量；不持有任何状态（便于测试与热重载）。
"""
...
def resolve_provider() -> str:
    ...
    优先级：
    1. ``LLM_PROVIDER`` 显式设置（大小写不敏感；不在 PROVIDERS 中时回退到 custom）
    2. ``SILICONFLOW_API_URL`` 含 ``siliconflow.cn`` → siliconflow
    3. ``SILICONFLOW_API_KEY`` 已设 → siliconflow（向后兼容）
    4. 默认 ``openai``

    Returns:
        provider name（``PROVIDERS`` 中的 key）
    """
    explicit = (os.getenv("LLM_PROVIDER") or "").strip().lower()
    if explicit:
        if explicit in PROVIDERS:
            return explicit
        # 未识别的 provider 名称按 custom 处理（让用户自填 URL）
        return "custom"
...
def get_provider_summary() -> Dict[str, Any]:
    ...
    Raises:
        ConfigError: 当 custom 预设缺少必填环境变量时。
    """
