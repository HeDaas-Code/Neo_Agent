...
class ScheduleManager:
        ...
        **kwargs
        ...
        if check_similarity:
            from src.prefrontal.schedule.schedule_similarity_checker import (
                ScheduleSimilarityChecker, get_schedules_on_same_day
            )
    ...
    def add_schedule(self, **kwargs) -> Tuple[bool, Optional[Schedule], str]:
        """
        日程添加的便捷入口（与 create_schedule 等价，供神经系统 channel 使用）。

        Returns:
            (是否成功, 日程对象或None, 消息)
        """
        return self.create_schedule(**kwargs)
    ...
    def list_schedules(
        ...
        如果提供 start_time / end_time，则委托给 get_schedules_by_time_range；
        否则返回按开始时间排序的日程列表。
        ...
        if start_time and end_time:
            return self.get_schedules_by_time_range(
                start_time, end_time,
