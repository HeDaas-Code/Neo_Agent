...
│       └── 更新基础知识
│
├── expression_style.py      # 表达风格管理（约500行）
│   └── ExpressionStyleManager # 个性化表达风格管理器
│       ├── 智能体表达管理
│       ├── 用户习惯学习
│       └── 提示词格式化
│
...
**分析方式**：
- 印象生成：基于角色性格和对话内容，生成对用户的详细印象
- 情感评分：维护一个累积的情感分数，用于表示当前关系状态
  - 初始评估：在对话满 5 轮后进行第一次评估，生成 0-35 分的基础分数
  - 更新评估：此后每经过 15 轮对话，根据最新印象对分数进行 -3 至 +3 的增量调整
  - 分数边界：累积分数始终被限制在 0-100 分之间，不会低于 0 或超过 100
  - 分数区间（基于当前累积分数）：
    - 0-20分：非常负面（冷淡、敌对、无礼等）
    - 21-40分：较负面（不够友好、缺乏兴趣等）
    - 41-60分：中性（普通交流，无明显倾向）
    - 61-80分：较正面（友好、有趣、积极等）
    - 81-100分：非常正面（热情、信任、深度交流等）
...
**职责**：管理智能体的个性化表达和学习用户的表达习惯

**核心功能**：
- 智能体表达管理：定义和管理智能体的个性化表达方式（如 'wc'、'hhh'）
- 用户习惯学习：自动识别和总结用户的表达习惯
- 提示词集成：将表达风格格式化后注入到对话提示词中
...
**数据库表**：
- `agent_expressions` - 存储智能体的个性化表达
- `user_expression_habits` - 存储学习到的用户表达习惯

### BaseKnowledge（基础知识管理器）

**职责**：管理智能体的核心基础知识，这些知识具有最高优先级且不可被覆盖

**核心特性**：
- 绝对权威性：基础知识拥有最高优先级
- 不可覆盖：即使与现实相悖也以该知识为准
- 直接注入：不以上下文形式存在，而是直接嵌入到提示词中
