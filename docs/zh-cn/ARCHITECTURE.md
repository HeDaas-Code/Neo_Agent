...
│                                                              │
│  ┌────────────────┐  ┌────────────────┐  ┌──────────────┐ │
│  │  ChatAgent     │  │ EventManager   │  │EmotionAnalyzer│ │
│  │  (对话代理)    │  │ (事件管理)     │  │ (情感分析)    │ │
│  └────────┬───────┘  └────────┬───────┘  └──────────────┘ │
│           │                   │                              │
│           │  ┌────────────────┴──────────────────┐          │
...
事件驱动系统为智能体提供了预先设置事件的能力，使得系统可以响应外部触发的事件。事件分为两大类型：

1. **通知型事件（Notification Event）**：智能体接收外部信息并向用户说明
2. **任务型事件（Task Event）**：多智能体协作完成复杂任务
...
class EventManager:
    ...
    def create_event(self, title, description, event_type, priority):
        """创建新事件"""
        event = Event(
    ...
    def get_pending_events(self, limit=10):
        """获取待处理事件，按优先级排序"""
        return self._query_events(status=EventStatus.PENDING, limit=limit)
```

##### MultiAgentCoordinator（多智能体协调器）

多智能体协调器负责任务型事件的处理，通过多个专业子智能体协作完成任务：
...
class MultiAgentCoordinator:
    ...
    def __init__(self):
        self.understanding_agent = SubAgent('理解智能体')
        self.planning_agent = SubAgent('规划智能体')
        self.execution_agent = SubAgent('执行智能体')
        self.verification_agent = SubAgent('验证智能体')
    
    def process_task_event(self, task_event):
        """处理任务型事件"""
        # 阶段1：理解任务
        understanding = self.understanding_agent.analyze(task_event)
        
        # 阶段2：制定计划
        plan = self.planning_agent.create_plan(understanding)
        
        # 阶段3：执行步骤
        results = []
        for step in plan.steps:
            result = self.execution_agent.execute(step)
            results.append(result)
        
        # 阶段4：验证结果
        verification = self.verification_agent.verify(results, task_event)
...
class InterruptQuestionTool:
    ...
    def ask_user(self, question, context=None):
        ...
        answer = self.callback_func(formatted_question)
        return answer
```
...
def process_notification_event(self, event):
    ...
    # 3. 记录日志
    self.event_manager.add_event_log(event.event_id, 'explanation', explanation)
    
    # 4. 更新状态
    self.event_manager.update_event_status(event.event_id, EventStatus.COMPLETED)
...
1. **策略模式**：不同事件类型使用不同的处理策略
2. **责任链模式**：多智能体协作的任务传递
3. **观察者模式**：进度回调机制
4. **工厂模式**：事件创建

更多详细信息请参考[事件系统文档](EVENT_SYSTEM.md)和[事件系统架构图](ARCHITECTURE_EVENT_SYSTEM.md)。
