...
EVALUATION_STAGES = {
    'initial': {
        'trigger': '对话满5轮',
        'analysis_scope': '前5轮对话（10条消息）',
        'score_range': (0, 35),
        'description': '基于初次见面的印象生成初始评分'
    },
    'update': {
        'trigger': '每15轮对话',
        'analysis_scope': '最近15轮对话（30条消息）',
        'score_change_range': (-3, 3),
        'description': '根据最近表现评估并调整累计分数'
    }
}
```
...
SCORING_SYSTEM = {
    'initial_evaluation': {
        'range': '0-35分',
        'criteria': {
            '0-10': '非常负面的初印象（冷淡、敌对、无礼等）',
            '11-20': '较负面的初印象（不够友好、态度消极等）',
            '21-28': '中性初印象（普通的开始，无明显特点）',
            '29-32': '较正面的初印象（友好、有礼貌等）',
            '33-35': '非常正面的初印象（热情、真诚、令人愉快等）'
        }
    },
    'update_evaluation': {
        'change_range': '-3到+3分',
        'criteria': {
            '-3': '最近表现很差（态度恶化、不尊重、冷漠等）',
            '-2': '表现较差（不够友好、兴趣降低等）',
            '-1': '略有不足（小问题、轻微不愉快等）',
            '0': '保持现状（无明显变化）',
            '+1': '略有改善（更友好、更积极等）',
            '+2': '表现较好（明显改善、增进了解等）',
            '+3': '表现很好（大幅改善、深入交流、建立信任等）'
        }
    },
    'cumulative_score': {
        'range': '0-100分',
        'boundaries': '累计分数始终限制在0-100之间',
        'interpretation': {
            '0-20': '非常负面（冷淡、敌对、无礼等）',
            '21-40': '较负面（不够友好、缺乏兴趣等）',
            '41-60': '中性（普通交流，无明显倾向）',
            '61-80': '较正面（友好、有趣、积极等）',
            '81-100': '非常正面（热情、信任、深度交流等）'
        }
    }
}
```
...
```python
initial_prompt = f"""
你是AI角色"{character_name}"，这是你和用户的初次见面。
请基于你的角色设定和最近5轮对话，生成你对用户的初步印象。
...
```python
update_prompt = f"""
你是AI角色"{character_name}"，你们已经有过交流了。
请基于最近15轮对话，更新你对用户的印象。
...
│
├── 右侧框架 (Frame)
│   ├── 评分圆环图 (EmotionImpressionDisplay)
│   ├── 时间线图 (TimelineCanvas)
│   └── 统计信息 (Frame)
│
└── 弹出窗口
