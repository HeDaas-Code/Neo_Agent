...
- **备份恢复**：完整的数据导入导出
- **查询优化**：高效的数据检索
- **设定迁移**：快速导出和导入智能体设定，支持跨环境迁移
  - 导出 .env 配置和数据库数据到单一 JSON 文件
  - 支持选择性导出（可自定义导出的数据类别）
  - 导入时自动备份现有配置
  - 可视化预览导入内容
...
### 📆 智能日程管理
- **三种日程类型**：
  - 周期日程：可重复的固定日程（如每周课程表）
  - 预约日程：用户提及或意图识别创建的日程
  - 临时日程：LLM在空闲时段自动生成的补充日程
- **智能冲突检测**：自动检测时间冲突，高优先级日程自动覆盖低优先级
- **优先级系统**：CRITICAL（周期）> HIGH > MEDIUM（预约）> LOW（临时）
...
├── settings_migration.py       # 设定迁移管理
├── settings_migration_gui.py   # 设定迁移GUI
...
### 设定迁移与恢复
1. **导出设定**：在主界面选择"📦 设定迁移"选项卡
   - 选择要导出的数据类别（基础知识、实体知识、记忆、情感历史等）
   - 可选是否包含 .env 配置
   - 点击"导出设定"保存为 JSON 文件
2. **导入设定**：
   - 在导入选项卡中选择导出的 JSON 文件
   - 点击"预览导入内容"查看文件详情
   - 选择要导入的数据类别
   - 可选是否覆盖现有数据（谨慎使用）
   - 点击"执行导入"完成导入
