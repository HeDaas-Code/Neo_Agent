...
### 📆 智能体日程管理
- **三种日程类型**：
  - 周期日程：固定重复的日程（如周一到周五的课程表）
  - 预约日程：用户提及或意图识别的日程（如周三下午开会）
  - 临时日程：LLM在空隙中添加的随机活动（如今晚看月亮）
- **自动优先级管理**：高优先级日程自动替换低优先级冲突日程
- **冲突检测**：智能检测时间重叠，防止日程冲突
- **对话集成**：日程信息作为上下文，智能体可在对话中自然提及
- **灵活重复模式**：支持每天、每周、工作日、周末、每月等多种重复方式
- **可视化管理**：GUI界面查看、添加、编辑和删除日程
