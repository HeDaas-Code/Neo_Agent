...
- **持久化存储**：情感数据保存到emotion_data.json，支持历史追踪
...
- **多标签页**：系统信息、短期记忆、长期记忆、理解阶段、知识库、Debug日志、控制面板
...
├── emotion_analyzer.py        # 情感关系分析模块（NEW!）
├── debug_logger.py            # Debug日志管理器
├── gui_debug.py               # 基础GUI界面
├── gui_enhanced.py            # 增强版GUI界面（推荐使用）
├── start_gui.py               # 快速启动脚本
├── test_system.py             # 系统测试脚本
├── test_base_knowledge.py     # 基础知识库测试脚本
├── test_emotion.py            # 情感分析测试脚本（NEW!）
├── test_emotion_debug.py      # 情感分析Debug测试（NEW!）
├── test_auto_emotion.py       # 自动情感分析测试（NEW!）
├── test_auto_emotion_debug.py # 自动情感分析Debug测试（NEW!）
├── memory_data.json           # 短期记忆数据（自动生成）
├── longmemory_data.json       # 长期记忆数据（自动生成）
├── knowledge_base.json        # 知识库数据（自动生成）
├── base_knowledge.json        # 基础知识库数据（自动生成）
├── emotion_data.json          # 情感分析数据（自动生成，NEW!）
├── debug.log                  # Debug日志文件（自动生成）
├── README.md                  # 本文件
├── DEBUG_UPDATE.md            # Debug和基础知识库更新说明
├── AUTO_EMOTION_DEBUG.md      # 自动情感分析Debug文档（NEW!）
└── AUTO_EMOTION_QUICKREF.md   # 情感分析快速参考（NEW!）
```
...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐           │
│  │ memory_data  │  │ knowledge_   │  │ emotion_     │  + 3 more │
│  │ .json        │  │ base.json    │  │ data.json    │           │
│  └──────────────┘  └──────────────┘  └──────────────┘           │
└───────────────────────────────────────────────────────────────────┘
                                │
┌───────────────────────────────▼───────────────────────────────────┐
...
#### Q: 短期记忆会丢失吗？
**A**: 不会。短期记忆保存在`memory_data.json`文件中，即使重启程序也会自动加载。超过20轮的对话会被归档到长期记忆，不会丢失。
...
#### Q: 如何手动删除某条知识？
**A**: 
- **方法1**：直接编辑`knowledge_base.json`文件，找到对应UUID删除
...
3. 确认基础知识已添加到`base_knowledge.json`
4. 查看知识上下文是否包含基础知识
...
**A**: 基础知识设计为不可修改。如需修改：
1. **推荐**：通过GUI删除后重新添加
2. **直接编辑**：修改`base_knowledge.json`文件
...
1. 确认已进行过至少一次情感分析
2. 切换到"💖 情感关系"标签页
3. 检查`emotion_data.json`文件是否存在
4. 点击"🔍 分析情感关系"手动触发
5. 查看GUI右下角是否有错误提示

#### Q: 如何重置情感数据？
**A**: 
- **方法1**：删除`emotion_data.json`文件
- **方法2**：手动编辑文件，清空数组`[]`
- **注意**：重置后会丢失所有情感分析历史
...
**A**: 备份以下JSON文件：
```
memory_data.json        # 短期记忆
longmemory_data.json    # 长期记忆
knowledge_base.json     # 知识库
base_knowledge.json     # 基础知识库
emotion_data.json       # 情感数据
```
定期复制这些文件到安全位置。
